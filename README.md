...
**爱加速代理网关（LoveToSpeedup）** 是一个基于[爱加速](https://www.ajiasu.com)国内节点的 Docker 化 HTTP 代理网关。它把爱加速 Linux 客户端封装为可被远程调用的 HTTP 代理服务，并配套提供 Flask Web 面板与 REST API，让你能方便地给宿主机、其他 Docker 容器或定时任务按需分配代理。

整个项目围绕「**多账户、多端口并行**」设计：

- **主端口（10801）**：对应原版行为，固定端口、按需获取——通过面板或 API `acquire` 拉起一个带有效期的代理，到期自动断开。
- **额外端口槽（10802–10809）**：每个槽绑定一个独立爱加速账户（可选地区），各自拥有独立的 ajiasu 实例与转发端口，因此多个账户可同时在线。槽位分两种模式：
  - **短期（short）**：带 TTL，用完到期自动释放；
  - **长期（long）**：常驻在线，守护线程每 10 秒巡检，掉线自动重连。
- **账户互斥**：同一爱加速账户同时只允许一个活动连接，网关在全局做排他校验，避免主端口与额外槽「抢账户」。

其他能力：面板 / API 登录密码保护（支持在线改密，API Key 同步）、按地区获取出口 IP、端口槽掉线自连、完全离线构建（`vendor/` 内置客户端二进制与全部 pip 依赖，适用于无法访问外网的部署环境）。
